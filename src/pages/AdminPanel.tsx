...
const AdminPanel = () => {
  ...
  const lifelineIcons = { FIFTY_FIFTY: Scissors, ASK_AUDIENCE: Users, PHONE_FRIEND: Phone };
  const lifelineLabels = { FIFTY_FIFTY: '50:50', ASK_AUDIENCE: 'Ask Audience', PHONE_FRIEND: 'Phone a Friend' };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex gap-2 mb-6">
          <button
            ...
            className={`px-6 py-3 rounded-xl font-display font-bold text-sm transition-all ${
              tab === 'control' ? 'bg-primary text-primary-foreground glow-blue' : 'bg-secondary/50 text-muted-foreground hover:text-foreground'
            }`}
          >
            <Zap size={16} className="inline mr-2" />Game Control
          </button>
          <button
            onClick={() => setTab('questions')}
            className={`px-6 py-3 rounded-xl font-display font-bold text-sm transition-all ${
              tab === 'questions' ? 'bg-primary text-primary-foreground glow-blue' : 'bg-secondary/50 text-muted-foreground hover:text-foreground'
            }`}
          >
            <BookOpen size={16} className="inline mr-2" />Question Sets
          </button>
        </div>

        {/* Questions Tab */}
        {tab === 'questions' && <QuestionSetManager />}
            ...
            {!session && (
              <div className="glass-panel p-12 text-center">
                ...
                <div className="grid gap-4 max-w-md mx-auto">
                  {questionSets.map(qs => (
                    <button key={qs.id} onClick={() => createSession(qs.id)} className="admin-btn-primary w-full">
                      <Play className="inline-block mr-2" size={20} />
                      {qs.name} ({qs.questions.length} questions)
                    </button>
                  ))}
                  {questionSets.length === 0 && (
                    <p className="text-muted-foreground font-body">No question sets yet. Create one in the "Question Sets" tab.</p>
                  )}
                </div>
              </div>
            )}
                ...
                <div className="lg:col-span-2 space-y-6">
                  ...
                  <div className="glass-panel p-6">
                    <h2 className="text-lg font-display font-bold text-foreground mb-4 flex items-center gap-2">
                      <span className="w-2 h-2 rounded-full bg-success animate-pulse" />Live Controls
                    </h2>
                    <div className="grid grid-cols-2 gap-4">
                      {session.status === 'WAITING' && (
                        <button onClick={startGame} className="admin-btn-success col-span-2">
                          <Play className="inline-block mr-2" size={20} />Start Game
                        </button>
                      )}
                      {isPlaying && (
                        <>
                          <button onClick={nextQuestion} disabled={!canNext} className={`admin-btn-primary ${!canNext ? 'opacity-40 cursor-not-allowed' : ''}`}>
                            <SkipForward className="inline-block mr-2" size={20} />Next Question
                          </button>
                          <button onClick={revealAnswer} disabled={!canReveal} className={`admin-btn-gold ${!canReveal ? 'opacity-40 cursor-not-allowed' : ''}`}>
                            <Eye className="inline-block mr-2" size={20} />Reveal Answer
                          </button>
                        </>
                      )}
                      {isPlaying && (
                        <button onClick={finishGame} className="admin-btn-danger col-span-2">
                          <Square className="inline-block mr-2" size={20} />End Game
                        </button>
                      )}
                      {isFinished && (
                        <button onClick={resetGame} className="admin-btn-primary col-span-2">
                          <RotateCcw className="inline-block mr-2" size={20} />New Game
                        </button>
                      )}
                    </div>
                  </div>
                  ...
                  {overlayState.currentQuestion && (
                    <div className="glass-panel-gold p-6">
                      ...
                      <div className="bg-background/50 rounded-lg p-4 mb-4">
                        ...
                        <p className="text-foreground font-display font-semibold text-lg">{overlayState.currentQuestion.text}</p>
                      </div>
                      <div className="grid grid-cols-2 gap-3">
                        {overlayState.currentQuestion.answers.map((ans, i) => {
                          ...
                          return (
                            <div key={ans.id} className={`p-3 rounded-lg border transition-all ${
                              isHidden ? 'opacity-20 border-border' :
                              isCorrect ? 'border-success bg-success/20 glow-green' :
                              'border-border bg-background/30'
                            }`}>
                              <span className="text-accent font-display font-bold mr-2">{String.fromCharCode(65 + i)}:</span>
                  ...
                  <div className="glass-panel p-6">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-accent font-display font-bold">{LEVEL_AMOUNTS[session.currentLevel] || '$0'}</span>
                      </div>
                    </div>
                  </div>
                </div>
